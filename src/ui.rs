...
pub fn draw_main_menu(frame: &mut Frame) {
    ...
    let menu_options = Paragraph::new(vec![
        ...
        .block(Block::default()
            .borders(Borders::ALL)
            .title(Span::styled("Main Menu", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)))
        )
        .wrap(Wrap { trim: true });
...
pub fn draw_select_file(frame: &mut Frame) {
    ...
    let input_prompt = Paragraph::new(vec![
        Line::from(Span::styled("Enter the file path:", Style::default().fg(Color::LightBlue))),
    ])
        .block(Block::default()
            .borders(Borders::ALL)
            .title(Span::styled("Select File", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)))
        )
        .wrap(Wrap { trim: true });

    // Footer-Info
    let footer = Paragraph::new(Line::from(vec![
        Span::styled("Press Enter to confirm the path.", Style::default().fg(Color::Gray)),
    ]))
        .block(Block::default().borders(Borders::TOP));
